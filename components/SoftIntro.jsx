...
export default function SoftIntro() {
  ...
  return (
    <section>
      <motion.h1
        className={`${lexend_font.className} text-6xl lg:text-8xl mb-10 text-black bg-white w-fit`}
      ...
      <motion.p className={`${lexend_font2.className} text-lg mb-10`}>
        a passionate technologist based in
        <motion.a
          className={`${locHover ? 'text-black' : 'text-white'} text-4xl`}
      ...
      >
        {hovering && (
          <motion.div
            className="absolute w-40 h-40 bg-cyan-300/30 rounded-full pointer-events-none blur-2xl opacity-80"
        ...
        {!readBio ? (
          <motion.h2
            className="text-lg relative z-10 animate-bounce text-blue-200"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
